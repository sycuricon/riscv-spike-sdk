...
process:
	...
	if (sk->sk_state == TCP_NEW_SYN_RECV) {
		...
		if (!tcp_filter(sk, skb)) {
			...
			// PEC PATCH
            // printk("sk_state_change: 0x%lx", sk->sk_state_change);
            // printk("sk_data_ready: 0x%lx", sk->sk_data_ready);
            // printk("sk_write_space: 0x%lx", sk->sk_write_space);
            // printk("sk_backlog_rcv: 0x%lx", sk->sk_backlog_rcv);
            // printk("sk_destruct: 0x%lx", sk->sk_destruct);
#ifdef CONFIG_PEC_CFI
            unsigned long long diff = (void*)(&(nsk->sk_state_change)) - (void *)nsk;
            unsigned long long sk_addr = (unsigned long long)sk + diff;
            unsigned long long nsk_addr = (unsigned long long)nsk + diff;
            unsigned long long decrypt, encrypt;
            //printk("before:nsk->sk_state_change: 0x%lx", *(unsigned long long *)(nsk_addr));
            CRDAK(decrypt, *(unsigned long long *)(sk_addr), sk_addr);
            //printk("decrypt: 0x%lx", decrypt);
            CREAK(encrypt, decrypt, nsk_addr);
            *(unsigned long long *)(nsk_addr) = encrypt;
            //printk("after:nsk->sk_state_change: 0x%lx", *(unsigned long long *)(nsk_addr));
#endif
		} else {

		  ...
		  unsigned int key_len)
{
	 ...
	 */
	// PEC PATCH: crypto
#ifdef CONFIG_PEC_CRYPTO
    // encrypt decryption key
    CREBK(*(u64 *)&ctx->key_dec[0], *(u64 *)&ctx->key_enc[key_len + 24], &ctx->key_dec[0]);
    CREBK(*(u64 *)&ctx->key_dec[2], *(u64 *)&ctx->key_enc[key_len + 26], &ctx->key_dec[2]);
    for (i = 4, j = key_len + 20; j > 0; i += 4, j -= 4) {
        encode = ((u64)inv_mix_columns(ctx->key_enc[j + 1]) << 32) | inv_mix_columns(ctx->key_enc[j]);
        CREBK(*(u64 *)&ctx->key_dec[i], encode, &ctx->key_dec[i]);
        encode = ((u64)inv_mix_columns(ctx->key_enc[j + 3]) << 32) | inv_mix_columns(ctx->key_enc[j + 2]);
        CREBK(*(u64 *)&ctx->key_dec[i + 2], encode, &ctx->key_dec[i + 2]);
	}
    CREBK(*(u64 *)&ctx->key_dec[i], *(u64 *)&ctx->key_enc[0], &ctx->key_dec[i]);
    CREBK(*(u64 *)&ctx->key_dec[i + 2], *(u64 *)&ctx->key_enc[2], &ctx->key_dec[i + 2]);

    // encrypt encryption key
    for (i = 0; i < AES_MAX_KEYLENGTH_U32; i += 2)
        CREBK(*(u64 *)&ctx->key_enc[i], *(u64 *)&ctx->key_enc[i], &ctx->key_enc[i]);
#else

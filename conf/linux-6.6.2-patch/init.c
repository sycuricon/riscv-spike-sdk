...
// PEC PATCH: page table
#ifdef CONFIG_PEC_PGTABLE
static void __init encrypt_initmm_pgd(void)
{
    void *encrypted_pgd;
    CREAK(encrypted_pgd, init_mm.pgd, &(init_mm.pgd));
    init_mm.pgd = encrypted_pgd;
}

void __init random_init_mm_pgd(void)
{
    void *pgd = (void *)__get_free_page(GFP_KERNEL);
    memcpy(pgd, swapper_pg_dir, PAGE_SIZE);
    CREAK(init_mm.pgd, pgd, &(init_mm.pgd));
    csr_write(CSR_SATP, virt_to_pfn(pgd) | satp_mode);
}
#endif

void __init clear_swapper_pg_dir(void)
{

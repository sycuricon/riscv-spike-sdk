...
int mpi_powm_pec(MPI res, MPI base, MPI exp, MPI mod);
...
static inline void encrypt_key(MPI key)
{
    int i, size = key->nlimbs;
    for (i = 0; i < size; ++i)
        CREBK(key->d[i], key->d[i], &(key->d[i]));
}
#endif

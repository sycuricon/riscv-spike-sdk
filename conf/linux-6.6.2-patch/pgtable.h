...
static inline pgd_t *pgd_offset_pgd(struct mm_struct *mm, unsigned long address)
{
	// PEC PATCH: page table
#ifdef CONFIG_PEC_PGTABLE
    pgd_t *decrypted_pgd;
    CRDAK(decrypted_pgd, mm->pgd, &(mm->pgd));
    return (decrypted_pgd + pgd_index(address));
#else
 ...
 */
#ifndef pgd_offset
#define pgd_offset(mm, address)		pgd_offset_pgd(mm, (address))
